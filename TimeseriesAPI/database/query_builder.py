...
class QueryBuilder:
    ...
    def _format_timestamp(self, timestamp: Any) -> str:
        """
        Format timestamp for SQL query
        Handles both Unix timestamps (milliseconds) and ISO datetime strings
        Returns proper TIMESTAMP literal for Presto
        """
        from datetime import datetime
        
        if isinstance(timestamp, (int, float)):
            # Unix timestamp in milliseconds - convert to datetime string
            timestamp_seconds = int(timestamp) / 1000.0
            dt = datetime.utcfromtimestamp(timestamp_seconds)
            datetime_str = dt.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]  # Trim to milliseconds
            return f"TIMESTAMP '{datetime_str}'"
        elif isinstance(timestamp, str):
            # ISO format string - wrap in TIMESTAMP literal
    ...
    def _get_aggregation_function(self, granularity: Optional[str]) -> str:
        ...
        #         raise ValueError(f"Unsupported time unit: {unit}")
            ...
            # Timestamp column is already TIMESTAMP type - use to_unixtime
            if unit in ["hour", "hours"]:
                return f"CAST(TO_UNIXTIME(DATE_TRUNC('hour', {timestamp_col})) * 1000 AS VARCHAR)"
            elif unit in ["minute", "minutes"]:
                return f"CAST(TO_UNIXTIME(DATE_TRUNC('minute', {timestamp_col})) * 1000 AS VARCHAR)"
            elif unit in ["day", "days"]:
                return f"CAST(TO_UNIXTIME(DATE_TRUNC('day', {timestamp_col})) * 1000 AS VARCHAR)"
            else:
                raise ValueError(f"Unsupported time unit: {unit}")
