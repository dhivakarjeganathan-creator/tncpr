"""
Manual test script for validating /timeseries API endpoint (without table parameter)
Table is automatically determined from metrics via metricsandtables table
...
ENDPOINT = f"{BASE_URL}/api/v1/timeseries"
...
def print_response(response: requests.Response, show_data: bool = False):
    ...
    print("\n" + "="*80)
    print("API REQUEST - /timeseries (table determined from metrics)")
    print("="*80)
    ...
    try:
        ...
        if is_flat_array:
            ...
            print("\n" + "-"*80)
            print("QUERY RESULTS (Flat Array Format)")
            print("-"*80)
            ...
            if len(data) > 0:
                # Print sample data
                if show_data:
                    print(f"\nSample Data (showing first {min(3, len(data))} records):")
                    for i, record in enumerate(data[:3]):
                        print(f"\n  Record {i+1}:")
                        print(f"    metric: {record.get('metric', 'N/A')}")
                ...
                print(f"\nUnique Metrics: {', '.join(sorted(unique_metrics))}")
            else:
                print("\nNote: No data returned (empty array - database may be empty or no table found for metrics)")
        else:
            ...
            print("QUERY RESULTS")
            print("-"*80)
            print(f"Table: {data.get('table', 'N/A')} (determined from metrics)")
            ...
            if show_data and "data" in data and len(data["data"]) > 0:
                ...
                print("\nNote: No data returned (database may be empty or no table found for metrics)")
...
def test_case_1_basic_metrics():
    ...
    print_test_header("Basic Query - Metrics Only (table determined from metrics)")
    
    params = {
        "metrics": "ranmarket_endc_sessions_rn,ranmarket_intra_cu_ho_attempts_number,ranmarket_ul_gtp_data_volume_rn_mb"
    }
...
def test_case_2_time_range():
    ...
    params = {
        "metrics": "ranmarket_drop_rate_pct_rn,ranmarket_attach_success_rate_pct_rn,ranmarket_dl_tput_rn_mbps",
        "start": "1749992400000",
        "end": "1750057199000"
    }
...
def test_case_4_multiple_entity_values():
    ...
    params = {
        "metrics": "ranmarket_drop_rate_pct_rn,ranmarket_attach_success_rate_pct_rn",
        "searchByProperties": "resource.id==143,144,145,146,147",
        "start": "1749992400000",
        "end": "1750057199000"
    }
...
def test_case_5_aggregation_example1():
    ...
    params = {
        "metrics": "ranmarket_endc_sessions_rn,ranmarket_intra_cu_ho_attempts_number,ranmarket_ul_gtp_data_volume_rn_mb,ranmarket_dl_tput_rn_mbps,ranmarket_ul_tput_rn_mbps",
        "start": "1749992400000",
        "end": "1750057199000",
...
def test_case_6_properties_selection():
    ...
    params = {
        "metrics": "ranmarket_dl_tput_rn_mbps,ranmarket_ul_tput_rn_mbps",
        "start": "1749992400000",
        "end": "1750057199000",
...
def test_case_8_complex_example1_full():
    ...
    # Based on Example 1 from requirements - table determined from metrics
    params = {
...
def test_case_9_complex_example2_full():
    ...
    # Based on Example 2 from requirements - table determined from metrics
    params = {
...
def test_case_12_error_cases():
    ...
    try:
        response = requests.get(ENDPOINT, params=params, timeout=REQUEST_TIMEOUT)
        print(f"Status: {response.status_code} (Expected: 400 or 200)")
    except requests.exceptions.ConnectionError:
        print("ERROR: Cannot connect to API server")
        raise
    except requests.exceptions.Timeout:
    ...
    # No table found for metrics
    print("\n--- Test: No table found for metrics ---")
    params = {
        "metrics": "nonexistent_metric_xyz_12345"
    }
    try:
        response = requests.get(ENDPOINT, params=params, timeout=REQUEST_TIMEOUT)
        print(f"Status: {response.status_code} (Expected: 200 with empty array)")
        if response.status_code == 200:
            data = response.json()
            assert isinstance(data, list), "Response should be a flat array"
            print(f"Array Length: {len(data)} (Expected: 0)")
    except requests.exceptions.ConnectionError:
        print("ERROR: Cannot connect to API server")
        raise
    except requests.exceptions.Timeout:
...
def test_health_check():
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/v1/health", timeout=REQUEST_TIMEOUT)
...
def main():
    """Run all test cases"""
    print("\n" + "="*80)
    print("KPI TIMESERIES API - MANUAL VALIDATION TEST SUITE (/timeseries endpoint)")
    print("="*80)
    print("\nMake sure the API server is running on http://localhost:8003")
    print("Note: Table will be automatically determined from metrics via metricsandtables table")
